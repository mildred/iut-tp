...
package body p_fenbase is
 ...
 procedure ChangerStyleTexte ( 
        F               : in out TR_Fenetre; 
        NomElement      : in     String; 
        NouveauStyle : in     FL_TEXT_STYLE   )is 
  P : TA_Element;  
  begin
    P:=GetElement(F.PElements,NomElement);
    if P/=null then
  ...
  function EtatBoiteCocher (                   -- Permet de savoir si une Checkbox est 
        ...
        NomElement : in     String      )
    return boolean is                             -- Résultat : l'état du bouton
  ...
  ------------------------------------------------------------------------------
  Procedure ChangerImageBouton (              -- Met / Change l'image d'un bouton image
        F          : in out    TR_Fenetre;
        NomElement, NomImage : in     String      ) is
     P : TA_Element;  
  begin
    P:=GetElement(F.PElements,NomElement);
 ...
 ------------------------------------------------------------------------------
  function ClickDroit return boolean is
	-- renvoie vrai si on a pressé le bouton droit de la souris
  begin
